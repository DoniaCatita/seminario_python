...
Se deben manejar situaciones simples como la introducción de productos
duplicados o la eliminación de productos inexistentes.
"""
import random
products = {}
seguir = True
...
while (seguir):
    ...
    match op_chosen:
        ...
        case 2:
            print("Ingrese el nombre del producto y "
            ...
            if new_name in products:
                user_choice = input(f"Ya existe un producto con ese nombre. "
                                    f"Desea sobreescribirlo? \n"
                                    f"1. Sí \n"
                                    f"2. No \n"
                                    )
                if not user_choice.isnumeric() or not (1 <=
                                                       int(user_choice) <= 2):
                ...
                if not new_stock.isnumeric():
                    ...
                    print("Inventario actualizado.")
        case 3:
            user_input = input(f"Ingrese el nombre del producto que quiere "
                               f"eliminar (cc para cancelar): ")
        ...
        case 5:
            ...
            if user_input in products:
                user_choice = input(f"1.Agregar unidades \n"
                                    f"2.Descontar unidades \n")
                if user_choice.isnumeric():
                    user_choice = int(user_choice)
                match user_choice:
                    case 1:
                        un = input(f"Ingrese la cantidad de unidades que "
                                   f"quiere agregar: ")
                        if un.isnumeric():
                            un = int(un)
                ...
                print("Producto no encontrado.")
        case 6:
            user_input = input(f"Ingrese el nuevo nombre del inventario "
                               f"(cc para cancelar): ")
            if user_input == "cc":
                print("Operación cancelada.")
            else:
    ...
    while not ok:
        ...
        # seria mejor hacerlo con un match como arriba?
        if not user_choice.isnumeric() or not (1 <= int(user_choice) <= 2):
            ...
            ok = True
            if user_choice == 2:
                seguir = False  # podria usar while(true) y hacer un exit(0)
        print()
print("Inventario cerrado.")
